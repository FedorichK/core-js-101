...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${startSymbol}${min}, ${max}${endSymbol}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < reversedCCN.length; i += 1) {
    let digit = parseInt(reversedCCN[i], 10);
...
function getDigitalRoot(num) {
  let newNum = num;
  while (newNum > 9) {
    newNum = newNum.toString().split('').reduce((sum, current) => sum + parseInt(current, 10), 0);
  }
...
function isBracketsBalanced(str) {
  ...
  return Array.from(str).every((bracket) => {
    if (brackets[bracket]) {
      stack.push(bracket);
...
function getCommonDirectoryPath(pathes) {
  const path = pathes.reduce((a, b) => a.split('/').filter((dir, i) => dir === b.split('/')[i]).join('/'));
